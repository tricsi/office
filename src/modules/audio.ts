...
function createSound(props: SoundProps, ctx: AudioContext | OfflineAudioContext = CONTEXT): Sound {
    const wave = props[0];
...
function createOfflineContext(id: string, time: number, buff = buffers): OfflineAudioContext {
    const ctx = new OfflineAudioContext(1, BITRATE * time, BITRATE);
    ctx.addEventListener("complete", (e) => buff.set(id, e.renderedBuffer));
    return ctx;
}
...
export function mixer(id: string, volume = -1, ctx = CONTEXT): GainNode {
...
export function play(id: string, loop = false, gain = "master", buff = buffers, ctx = CONTEXT): AudioBufferSourceNode {
    if (buff.has(id)) {
        let src = ctx.createBufferSource();
        src.loop = loop;
        src.buffer = buff.get(id);
